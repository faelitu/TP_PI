...
public class MainSobel {
    public static void main(String[] args) throws Exception {
        ...
        while ((st = br.readLine()) != null) {
            ...
            if (count == 1) {
                bw.append("# "+nome+"Sobel.pgm created by Rafael Machado\n");
            }
        ...
        int[][] img_sobel = new int[height][width];
        for (int i = 1; i < height + 1; i++) {
            for (int j = 1; j < width + 1; j++) {
                ...
                img_sobel[i-1][j-1] = (int) round(sqrt(gradX*gradX + gradY*gradY));
                
                bw.append(String.valueOf(img_sobel[i-1][j-1]) + " ");
            }
            bw.append("\n");
        }
