...
public class MainRoberts {
    public static void main(String[] args) throws Exception {
        ...
        while ((st = br.readLine()) != null) {
            ...
            if (count == 1) {
                bw.append("# "+nome+"Roberts.pgm created by Rafael Machado\n");
            }
        ...
        int[][] img_roberts = new int[height][width];
        for (int i = 1; i < height + 1; i++) {
            for (int j = 1; j < width + 1; j++) {
                float gradX = (img[i-1][j-1]*XMatrix[0][0] +
                               img[i-1][j]*XMatrix[0][1] +
                               img[i][j-1]*XMatrix[1][0] +
                               img[i][j]*XMatrix[1][1]);
                
                float gradY = (img[i-1][j-1]*YMatrix[0][0] +
                               img[i-1][j]*YMatrix[0][1] +
                               img[i][j-1]*YMatrix[1][0] +
                               img[i][j]*YMatrix[1][1]);
                
                img_roberts[i-1][j-1] = (int) round(sqrt(gradX*gradX + gradY*gradY));
                
                bw.append(String.valueOf(img_roberts[i-1][j-1]) + " ");
            }
            bw.append("\n");
        }
